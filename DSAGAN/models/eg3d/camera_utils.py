...
class LookAtPoseSampler:
    ...
    """
    @staticmethod
    def sampleDSA(primary_angle, secondary_angle, DSP, device='cpu',x_delta=0,y_delta=0,z_delta=0):
        # dsa2world = campolar2rotation(primary_angle, secondary_angle, DSP, device)
        dsa2world = campolar2rotation(primary_angle, secondary_angle, DSP, device,x_delta=0,y_delta=0,z_delta=0)
        return dsa2world
    @staticmethod
...
def campolar2rotation(a, b, r, device='cpu',x_delta=0,y_delta=0,z_delta=0):
    # r = float(r)/ 157.7
    # # r = 4.32
    # theta = np.deg2rad(float(a))  # 极角
    # phi = np.deg2rad(float(b))  # 极坐�?    # x_camera = r * np.sin(theta) * np.cos(phi)
    # y_camera = r * np.sin(theta) * np.sin(phi)
    # z_camera = r * np.cos(theta)
    # # 相机坐标系的三个轴在世界坐标系中的方�?    # z_xc, z_yc, z_zc = -x_camera, -y_camera, -z_camera
    # if y_camera < 0:
    ...
    T = np.eye(4)
    T[:3, :3] = R
    T[:3, 3] = [x_camera+x_delta, y_camera+y_delta, z_camera+z_delta]
