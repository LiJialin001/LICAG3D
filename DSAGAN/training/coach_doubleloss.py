...
class Coach:
        ...
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                if self.global_step >= self.opts.add_triplanenet:
                    # self.net.module.triplanenet_encoder.requires_grad_(True)
                    self.net.triplanenet_encoder.requires_grad_(True)
                else:
                    # self.net.module.triplanenet_encoder.requires_grad_(False)
                    self.net.triplanenet_encoder.requires_grad_(False)
                if self.global_step < self.opts.stop_psp:
                    # self.net.module.psp_encoder.requires_grad_(True)
                    self.net.psp_encoder.requires_grad_(True)
                else:
                    # self.net.module.psp_encoder.eval().requires_grad_(False)
                ...
                if self.global_step < self.opts.stop_psp:
                    ...
                    if positive != None and self.global_step %100==0 and self.global_step>100000:
                ...
                if self.opts.use_wandb and batch_idx == 0:
                    self.wb_logger.log_images_to_wandb(x, y_hat, y_hat_psp, id_logs, prefix="train",
                                                       step=self.global_step, opts=self.opts)
    ...
    def validate(self):
        ...
        for batch_idx, batch in enumerate(self.test_dataloader):
            ...
            # Log images of first batch to wandb
            if self.opts.use_wandb and batch_idx == 0:
                self.wb_logger.log_images_to_wandb(x, y_hat, y_hat_psp, id_logs, prefix="test", step=self.global_step,
                                                   opts=self.opts)
    ...
    def configure_triplane_optimizers(self):
        # params = list(self.net.module.triplanenet_encoder.parameters())
...
##################
    # def configure_doublelet_optimizers(self):
    #     params = list(self.net.psp_encoder.parameters())
    #     if self.opts.optim_name_psp == 'adam':
    #         optimizer = torch.optim.Adam(params, lr=self.opts.learning_rate_psp)
    #     else:
...
###############doublelet_loss#################.
    ...
    def doublelet_loss(self, anchor, positive, loss_dict):
        ...
        id_logs = None
        loss_dict['loss_doublelet'] = float(loss)
        return loss, loss_dict, id_logs
#######################################
